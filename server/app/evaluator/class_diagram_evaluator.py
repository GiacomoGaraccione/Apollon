...
from app.evaluator.utils.class_diagram import convert_apollon_model_to_reference, SyntaxErrorType, AttributeType, Multiplicity, SemanticErrorType
import json
import sys
import re

class ClassDiagramEvaluator(BaseEvaluator):

    def validate_solution(self, solution):
        """Validate the provided class diagram solution"""
        ...
        try:
            ...
            for sol in solutions:
                ...
                if completeness > best_completeness:
                    ...
                    best_reference = reference
                    best = report
            semantic_errors = self._get_semantic_errors_from_report(best, best_reference)
            syntax_errors = self._get_syntax_errors_from_model(convert_apollon_model_to_reference(model), best_reference, best)
    ...
    def _get_syntax_errors_from_model(self, model, reference=None, report=None):
        ...
        try:
            ...
            for cl in model.get("classes", []):
                ...
                if not name:
                    errors.append({
                        "type": SyntaxErrorType.MISSING_CLASS_NAME._value_,
                        "message": "Class name is missing",
                        "element": cl
                    })
                elif name in names_seen:
                    errors.append({
                        "type": SyntaxErrorType.DUPLICATE_CLASS_NAME._value_,
                ...
                for attr in cl.get("attributes", []):
                    ...
                    if not attr_name:
                        errors.append({
                            "type": SyntaxErrorType.MISSING_ATTRIBUTE_NAME._value_,
                        ...
                        errors.append({
                            "type": SyntaxErrorType.DUPLICATE_ATTRIBUTE_NAME._value_,
                    ...
                    if cl.get("type") != "Enumeration":
                        if len(attr_types) == 0 or attr_types[0] == "":
                            errors.append({
                                "type": SyntaxErrorType.MISSING_ATTRIBUTE_TYPE._value_,
                                "message": f"Attribute '{attr.get('name')}' in class '{cl.get('name')}' has no type defined",
                                "attribute": attr,
                                "class": cl.get("name")
                            })  
                        else:
                            allowed_types = [t.value.lower() for t in AttributeType] + [en.lower() for en in enumeration_names]
                            if attr_types[0].lower() not in allowed_types:
                                errors.append({
                                    "type": SyntaxErrorType.INVALID_ATTRIBUTE_TYPE._value_,
                        ...
                        if len(attr_types) > 0 and attr_types[0] != "":
                            errors.append({
                                "type": SyntaxErrorType.ENUMERATION_TYPE_WITH_ATTRIBUTES._value_,
                    ...
                    for other_cl in model.get("classes", []):
                        ...
                        if other_name and other_name in attr_name and other_name != name:
                            ...
                            if not matching_report_attr or not matching_report_attr.get("allowsForeignKeyName", False):
                                errors.append({
                                    "type": SyntaxErrorType.FOREIGN_KEY_REFERENCE._value_,
                    ...
                    if attr_types and attr_types[0] and attr_types[0] in class_names:
                        ...
                        if typed_class.get("type") == "Enumeration":
                            for assoc in model.get("associations"):
                                ...
                                errors.append({
                                    "type": SyntaxErrorType.UNCONNECTED_ENUMERATION._value_,
                            ...
                            errors.append({
                                "type": SyntaxErrorType.CLASS_AS_ATTRIBUTE_TYPE._value_,
                ...
                if not class_used_in_association:
                    errors.append({
                        "type": SyntaxErrorType.UNCONNECTED_CLASS._value_,
                ...
                if cl.get("type", "Class") == "IntermediateClass":
                    ...
                    if len(connected_classes) != 2:
                        errors.append({
                            "type": SyntaxErrorType.INVALID_INTERMEDIATE_CLASS_CONNECTIONS._value_,
                ...
                if assoc.get("type") == "Default":
                    ...
                    if source_type != "Enumeration" and target_type != "Enumeration":
                        if source.get("multiplicities", [""]) == [""]:
                            errors.append({
                                "type": SyntaxErrorType.MISSING_ASSOCIATION_MULTIPLICITY._value_,
                                "message": f"Association '{assoc.get('name')}' has no source multiplicity defined",
                                "association": assoc,
                                "class": source.get("referenceClass", {}).get("name")
                            })
                        elif not self._get_multiplicity(source.get("multiplicities", [""])[0]):
                            errors.append({
                                "type": SyntaxErrorType.INVALID_ASSOCIATION_MULTIPLICITY._value_,
                        ...
                        elif not self._get_multiplicity(target.get("multiplicities", [""])[0]):
                            errors.append({
                                "type": SyntaxErrorType.INVALID_ASSOCIATION_MULTIPLICITY._value_,
                        ...
                        if not assoc.get("name", "").strip():
                            errors.append({
                                "type": SyntaxErrorType.MISSING_ASSOCIATION_NAME._value_,
                    ...
                    if source.get("referenceClass", {}).get("name", "") == target.get("referenceClass", {}).get("name", ""):
                        ...
                        if source_no_role and target_no_role:
                            errors.append({
                                "type": SyntaxErrorType.MISSING_RECURSIVE_ASSOCIATION_ROLE._value_,
    ...
    def _get_multiplicity(self, multiplicity):
        if re.fullmatch(f"0", multiplicity):
            ...
            return Multiplicity.ONE_TO_NUMERIC
        elif re.fullmatch(r"0.*\d+", multiplicity):
            return Multiplicity.ZERO_TO_NUMERIC
        elif re.fullmatch(r"\d+.*\d+", multiplicity):
            return Multiplicity.NUMERIC_TO_NUMERIC
        elif re.fullmatch(r"\d+.*\*", multiplicity):
            return Multiplicity.NUMERIC_TO_MANY
        
        return None

    def _get_semantic_errors_from_report(self, report, reference):
        errors = []
        try:
            for cl in reference.get("classes", []):
                ...
                for mc in report.get("matchingClasses", []):
                    if mc.get("referenceClass") == cl.get("name"):
                        ...
                        if not mc.get("correctType"):
                            errors.append({
                                "type": SemanticErrorType.CLASS_TYPE._value_,
                                "name": cl.get("name"),
                ...
                if not found:
                    errors.append({
                        "type": SemanticErrorType.MISSING_CLASS._value_,
                        "name": cl.get("name"),
                    ...
                    for attr in cl.get("attributes", []):
                        ...
                        if not attr_found:
                            errors.append({
                                "type": SemanticErrorType.MISSING_ATTRIBUTE._value_,
                                "class": cl.get("name"),
                                "name": attr.get("name"),
                            ...
                            if not ma.get("typesMatch"):
                                errors.append({
                                    "type": SemanticErrorType.ATTRIBUTE_TYPE._value_,
                                    "class": cl.get("name"),
                                    "name": attr.get("name"),
            ...
            for cl in report.get("forbiddenClasses", []):
                errors.append({
                    "type": SemanticErrorType.FORBIDDEN_CLASS._value_,
            ...
            for cl in report.get("matchingClasses", []):
                for fa in cl.get("forbiddenAttributes", []):
                    errors.append({
                        "type": SemanticErrorType.FORBIDDEN_ATTRIBUTE._value_,
            ...
            for assoc in reference.get("associations", []):
                ...
                if not found:
                    ...
                    if source_exists and target_exists:
                        errors.append({
                            "type": SemanticErrorType.MISSING_ASSOCIATION._value_,
                            "source": source,
                            "target": target,
            ...
            for assoc in report.get("matchingAssociations", []):
                ...
                if ref_assoc.get("name") != "":
                    ...
                    if sim < 0.7:
                        errors.append({
                            "type": SemanticErrorType.ASSOCIATION_NAME._value_,
                ...
                if (diagram_type == "ClassBidirectional" or diagram_type == "ClassUnidirectional") and assoc.get("source_pair", {}).get("diagramInfo", {}).get("multiplicity", "") not in assoc.get("source_pair", {}).get("referenceInfo", {}).get("multiplicities", []):
                    errors.append({
                        "type": SemanticErrorType.ASSOCIATION_MULTIPLICITY._value_,
                ...
                if not correct:
                    errors.append({
                        "type": SemanticErrorType.ASSOCIATION_TYPE._value_,
            ...
            for forbidden in report.get("forbiddenAssociations", []):
                errors.append({
                    "type": SemanticErrorType.FORBIDDEN_ASSOCIATION._value_,
