...
export function ForbiddenActorUseCaseAssociationForm(props: {
    ...
    return (
        <>
            {((actors.length + systems.length) === 0 || useCases.length === 0) ? <Alert icon={<IconExclamationCircle size={16} />} title="Insufficient elements" mb="md">
                You need at least one actor/external system and one use case to create forbidden associations.
            </Alert> : <>
                    ...
                    <Grid.Col span={8}>
                        <Fieldset legend="Existing Associations" style={{ width: "100%" }}>
                            {rels.length === 0 && <Alert icon={<IconExclamationCircle size={16} />} title="No use cases found!">
                                There are no forbidden associations between actors and use cases for this solution yet. You can create one by filling the form on the left.
                            </Alert>}
                                ...
                                {rels.map((a, index) => {
                                    return (
                                        <Card shadow="sm" padding="lg" radius="md" withBorder key={index} onClick={() => setCurrentRel(a)}
                                            ...
                                            <Text>{a.sourceId} <i>cannot</i> perform {a.targetId}</Text>
                                        </Card>
                                    )
                                })}
                            </Flex>}
