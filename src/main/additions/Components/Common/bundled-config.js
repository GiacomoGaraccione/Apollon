...
var labelRequired = function () {
    ...
    function check(node, reporter) {
        ...
        if (is$g(node, 'bpmn:Gateway') && !isForking(node)) {
            return;
        }

        if (is$g(node, 'bpmn:BoundaryEvent')) {
            return;
        }

        // ignore sub-processes
        if (is$g(node, 'bpmn:SubProcess')) {
        ...
        if (is$g(node, 'bpmn:SequenceFlow') && !hasCondition$1(node)) {
            return;
        }
...
function isCompensationBoundary(node) {
    ...
    return is$d(eventDefinitions[0], 'bpmn:CompensateEventDefinition');
}
...
var noDuplicateSequenceFlows = function () {
    ...
    function check(node, reporter) {
        ...
        if (!is$c(node, 'bpmn:SequenceFlow')) {
            return;
        }
...
var noImplicitEnd = function () {
    ...
    function isImplicitEnd(node) {
        ...
        if (is$a(node, 'bpmn:IntermediateThrowEvent') && isLinkEvent(node)) {
            return false;
        }
...
var noImplicitStart = function () {
    ...
    function isLinkEvent(node) {
        ...
        return eventDefinitions.length && eventDefinitions.every(
            definition => is$9(definition, 'bpmn:LinkEventDefinition')
        );
    }
    ...
    function isImplicitStart(node) {
        ...
        if (is$9(node, 'bpmn:IntermediateCatchEvent') && isLinkEvent(node)) {
            return false;
        }
...
var startEventRequired = function () {
    ...
    function hasStartEvent(node) {
        ...
        return (
            flowElements.some(node => is$7(node, 'bpmn:StartEvent'))
        );
    }
...
var subProcessBlankStartEvent = function () {
    ...
    function check(node, reporter) {
        ...
        if (!is$6(node, 'bpmn:SubProcess') || node.triggeredByEvent) {
            return;
        }
...
/**
 * Rule that reports boundary events being blank
 */

var blankBoundaryEvent = function () {
    function check(node, reporter) {
        if (!is$4(node, "bpmn:BoundaryEvent")) return
        if (!node.eventDefinitions) reporter.report(node.id, "Boundary event cannot be a blank event");
    }
...
const {
    isAny: isAny$1 } = require$$0;

/**
 * Rule that reports intermediate events being blank
 */

var blankIntermediateEvent = function () {
    function check(node, reporter) {
        if (!isAny$1(node, ["bpmn:IntermediateCatchEvent", "bpmn:IntermediateThrowEvent"])) return
        if (!node.eventDefinitions) reporter.report(node.id, "Intermediate event cannot be a blank event");
    }
...
/**
 * Rule that reports boundary events being attached to a task without an outgoing sequence flow
 */
var boundaryEventTaskFlow = function () {
    function check(node, reporter) {
        if (!is$3(node, "bpmn:BoundaryEvent")) return
        let attachedToRef = node.attachedToRef;
        let outgoing = attachedToRef.outgoing || [];
        if (outgoing.filter((flow) => is$3(flow, "bpmn:SequenceFlow")).length <= 0) reporter.report(attachedToRef.id, "A task with a boundary event must also have a regular outgoing sequence flow");
    }
...
/**
 * Rule that reports message flows being sent by the wrong elements
 */
var noIncorrectOutgoingMessageFlow = function () {
    function check(node, reporter) {
        if (!is$2(node, "bpmn:MessageFlow")) return
        let sourceRef = node.sourceRef;
        if (!isAny(sourceRef, ["bpmn:IntermediateThrowEvent", "bpmn:EndEvent", "bpmn:Participant", "bpmn:ServiceTask", "bpmn:SendTask"])) reporter.report(node.id, "Message flow is sent by an element that cannot send messages");
    }
...
/**
 * Rule that reports boundary events being attached to service tasks
 */
var serviceTaskBoundaryEvent = function () {
    function check(node, reporter) {
        if (!is$1(node, "bpmn:BoundaryEvent")) return
        let attachedToRef = node.attachedToRef;
        if (is$1(attachedToRef, "bpmn:ServiceTask")) reporter.report(node.id, "Boundary event cannot be attached to a service task");
    }
...
/**
 * A rule that checks whether not more than one start event exists per scope.
 */
var singleStartEvent = function () {
    ...
    function check(node, reporter) {
        ...
        if (blankStartEvents.length > 1) {
            ...
            reporter.report(node.id, type + ' has multiple start events');
        }
    }
...
const rules = {
    ...
    "single-blank-start-event": 0,
    "single-event-definition": "error",
    "start-event-required": "error",
    "sub-process-blank-start-event": "error",
    "superfluous-gateway": "error",
    "bipmin/blank-boundary-event": "error",
    "bipmin/blank-intermediate-event": "error",
    "bipmin/boundary-event-task-flow": "error",
    "bipmin/no-incorrect-outgoing-message-flow": "error",
    "bipmin/service-task-boundary-event": "error",
    "bipmin/single-start-event": "error"
};
...
cache['bpmnlint-plugin-bipmin/blank-boundary-event'] = blankBoundaryEvent;

cache['bpmnlint-plugin-bipmin/blank-intermediate-event'] = blankIntermediateEvent;

cache['bpmnlint-plugin-bipmin/boundary-event-task-flow'] = boundaryEventTaskFlow;

cache['bpmnlint-plugin-bipmin/no-incorrect-outgoing-message-flow'] = noIncorrectOutgoingMessageFlow;

cache['bpmnlint-plugin-bipmin/service-task-boundary-event'] = serviceTaskBoundaryEvent;

cache['bpmnlint-plugin-bipmin/single-start-event'] = singleStartEvent;
